...
echo "$($PSQL "SELECT course FROM students LEFT JOIN majors USING (major_id) INNER JOIN majors_courses USING (major_id) INNER JOIN courses USING (course_id) GROUP BY course HAVING count(student_id) = 1 ORDER BY course;")"
